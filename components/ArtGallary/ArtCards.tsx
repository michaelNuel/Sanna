  ...
  return (
    <div className="flex flex-col gap-1 mr-6 ml-6 px-5 py-4 bg-[#100a18] rounded-2xl">
      ...
      <div>
        <div className="flex gap-2">
          <Image alt="sol" height={20} width={20} src="/solana.svg" />
          <p className="text-purple-600 font-medium text-sm ">$20000 (0.3 ETH)</p>
        </div>
      </div>
      <button className="mt-2 text-white text-sm bg-[#9747FF] px-8 py-3 rounded-3xl">
        Donate NFT Here
      </button>
    </div>
  );
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
      <div className="bg-PrimaryColor rounded-lg p-5 shadow-lg">
        ...
        {walletAddress ? (
          <div>
            <p className="text-white">Your Balance: {userBalance} ETH</p>
            <p className="text-white">Your Address: {walletAddress.slice(0, 6)}...{walletAddress.slice(-4)}</p>
            <input
              type="number"
              placeholder="Enter donation amount"
              value={donationAmount}
...
const ArtCards = ({ type }: Props) => {
  ...
  const sendPayment = async (amount: string) => {
    ...
    const paymentAmount = web3.utils.toWei(amount, "ether");
  ...
  return (
    <div>
      <h2 className="text-white">Art Cards Type: {type}</h2> {/* Using type here */}
      <div className="bg-[#100a18] mt-5 rounded-2xl">
        <div className="flex justify-center gap-7 mx-4 ">
          <div className="h-[357px] w-[606px] bg-gray-400 my-4 rounded-2xl"></div>
          <div className="flex w-[602px] flex-col gap-5 my-4">
            <h3 className="text-white text-sm font-bold">Innercity Missions</h3>
            <p className="text-white text-xs font-normal opacity-70 leading-5">Lorem ipsum...</p>
            <div className="flex gap-2">
              <Image alt="sol" height={20} width={20} src="/solana.svg" />
              <p className="text-purple-600 font-medium text-sm ">$20000 (0.3 ETH)</p>
            </div>
